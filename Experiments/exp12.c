...
int get_sym_index(const char *s) {
    ...
    if (sym_count >= MAX_SYMBOLS) { fprintf(stderr, "Too many symbols\n"); exit(1); }
...
int items_equal_core(const ItemSet *a, const ItemSet *b) {
    // cores equal if for every (p,dot) in a there exists same (p,dot) in b and vice versa
    for (int i = 0; i < a->n; ++i) {
        int ap = a->items[i].p, adot = a->items[i].dot;
        int found = 0;
        for (int j = 0; j < b->n; ++j) {
            if (b->items[j].p == ap && b->items[j].dot == adot) { found = 1; break; }
        }
        if (!found) return 0;
    }
    for (int i = 0; i < b->n; ++i) {
        int bp = b->items[i].p, bdot = b->items[i].dot;
        int found = 0;
        for (int j = 0; j < a->n; ++j) {
            if (a->items[j].p == bp && a->items[j].dot == bdot) { found = 1; break; }
        }
        if (!found) return 0;
    }
    return 1;
...
void add_first(int X, int t) {
    ...
    if (FIRST_count[X] >= MAX_FIRST) { fprintf(stderr, "FIRST overflow\n"); exit(1); }
...
void closure(ItemSet *I) {
    ...
    while (changed) {
        ...
        for (int i = 0; i < I->n; ++i) {
            Item it = I->items[i];
            if (it.p < 0 || it.p >= prod_count) continue;
            Prod *p = &prods[it.p];
            if (it.dot < p->rhs_len) {
                int B = p->rhs[it.dot];
                if (!is_terminal[B]) {
                        ...
                        if (prods[prod_j].lhs == B) {
                            for (int li = 0; li < lan; ++li) {
                                ...
                                if (!contains_item(I, newit)) {
                                    if (I->n >= MAX_ITEMS) { fprintf(stderr, "ItemSet overflow\n"); exit(1); }
                                    I->items[I->n++] = newit; changed = 1;
                                }
                            }
                        }
                    }
    ...
    for (int i = 0; i < I->n; ++i) {
        ...
        if (it.dot < p->rhs_len && p->rhs[it.dot] == X) {
            ...
            if (!contains_item(J, nit)) {
                if (J->n >= MAX_ITEMS) { fprintf(stderr, "ItemSet overflow in goto\n"); exit(1); }
                J->items[J->n++] = nit;
            }
        }
    }
...
void build_LR1() {
    ...
    Item it0 = {0, 0, dollar}; // $ as lookahead
    I0.items[I0.n++] = it0;
    closure(&I0);
    C[C_count++] = I0;
    int changed = 1;
    while (changed) {
        ...
        for (int i = 0; i < C_count; ++i) {
            ...
            for (int X = 0; X < sym_count; ++X) {
                    ...
                    if (C[k].n == J.n) {
                        ...
                        for (int a = 0; a < J.n; ++a) {
                            ...
                            for (int b = 0; b < C[k].n; ++b)
                                if (J.items[a].p == C[k].items[b].p && J.items[a].dot == C[k].items[b].dot && J.items[a].la == C[k].items[b].la) { found=1; break; }
                ...
                if (exists == -1) {
                    if (C_count >= MAX_STATES) { fprintf(stderr, "Too many LR(1) states\n"); exit(1); }
                    C[C_count++] = J; changed = 1;
                }
            }
        }
    }
    ...
    for (int i = 0; i < C_count; ++i) {
        ...
        for (int j = i+1; j < C_count; ++j) {
            if (items_equal_core(&C[i], &C[j])) {
                // add items from C[j] whose (p,dot,la) not already present
                for (int a = 0; a < C[j].n; ++a) {
                    Item it = C[j].items[a];
                    int found = 0;
                    for (int b = 0; b < lalr_states[lalr_count].n; ++b)
                        if (lalr_states[lalr_count].items[b].p == it.p && lalr_states[lalr_count].items[b].dot == it.dot && lalr_states[lalr_count].items[b].la == it.la) {
                            found=1; break;
                        }
                    if (!found) {
                        if (lalr_states[lalr_count].n >= MAX_ITEMS) { fprintf(stderr, "LALR item overflow\n"); exit(1); }
...
// ACTION: for terminals and $, use encoding: shift s to state, reduce by prod r, accept, or error
...
void build_parsing_table() {
    // initialize
    for (int i = 0; i < lalr_count; ++i) for (int j = 0; j < sym_count; ++j) { ACTION[i][j].action = 0; ACTION[i][j].number = -1; GOTO[i][j] = -1; }
        ...
        for (int a = 0; a < I->n; ++a) {
            ...
            if (it.dot < p->rhs_len) {
                        ...
                        if (C[m].n == J.n) {
                            ...
                            for (int aa=0; aa<J.n; ++aa) {
                                ...
                                for (int bb=0; bb<C[m].n; ++bb)
                                    if (J.items[aa].p==C[m].items[bb].p && J.items[aa].dot==C[m].items[bb].dot && J.items[aa].la==C[m].items[bb].la) { f=1; break; }
                                if (!f) { all=0; break; }
                            }
                            if (all) { idx = m; break; }
                        }
                ...
                if (it.p == 0) {
                    ...
                    // reduce by production it.p on lookahead it.la
                    ACTION[s][it.la].action = 2; ACTION[s][it.la].number = it.p; // reduce by production p
                }
            }
        }
    }
...
void parse_input() {
    printf("Enter input tokens separated by spaces (terminals). Use 'id' or token names. End with $ or include it):\n");
    char line[1024];
...
void load_grammar() {
    ...
    if (!f) {
        ...
        for (int i = 0; lines[i]; ++i) {
            ...
            if (!arrow) arrow = strstr(buf, "->"); // tolerate both
            if (!arrow) {
                // try "->" with spaces
                arrow = strstr(buf, "->");
            }
            if (!arrow) {
                // tolerate "->" with spaces by finding first '-' then '>'
                char *dash = strchr(buf, '-');
                if (dash && dash[1] == '>') arrow = dash;
            }
            if (!arrow) {
                // tolerate "->" with spaces by splitting at first occurrence of '>'
                arrow = strstr(buf, ">");
            }
            if (!arrow) continue;
            // Ensure we get lhs and rhs properly by splitting at '->' or '>'
            char *lhs = buf;
            char *rhs;
            if (strstr(buf, "->")) {
                arrow = strstr(buf, "->");
            ...
            while (isspace((unsigned char)*lhs)) lhs++; char *end = lhs+strlen(lhs)-1; while (end>lhs && isspace((unsigned char)*end)) *end--='\0';
            while (isspace((unsigned char)*rhs)) rhs++; end = rhs+strlen(rhs)-1; while (end>rhs && isspace((unsigned char)*end)) *end--='\0';
            int L = get_sym_index(lhs);
            // tokenize rhs by spaces
            char tmp[256]; strcpy(tmp, rhs);
            char *tk = strtok(tmp, " ");
        ...
        while (fgets(line, sizeof(line), f)) {
            char *s = line; while (isspace((unsigned char)*s)) s++; if (*s=='\0' || *s=='#') continue;
            char *arrow = strstr(s, "->"); if (!arrow) arrow = strstr(s, "->"); if (!arrow) continue;
            *arrow='\0';
            char *lhs = s; char *rhs = arrow+2; char *end = lhs+strlen(lhs)-1; while (end>lhs && isspace((unsigned char)*end)) *end--='\0';
            end = rhs+strlen(rhs)-1; while (end>rhs && isspace((unsigned char)*end)) *end--='\0';
            int L = get_sym_index(lhs);
            prods[prod_count].lhs = L; prods[prod_count].rhs_len = 0;
            char tmp[256]; strcpy(tmp, rhs);
            char *tk = strtok(tmp, " \t\n");
    ...
    if (prod_count + 1 >= MAX_PROD) { fprintf(stderr, "Too many productions\n"); exit(1); }
    ...
    int dollar = find_sym("$"); if (dollar == -1) { dollar = get_sym_index("$"); is_terminal[dollar]=1; }
}
